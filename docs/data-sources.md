...
| `query.lua` WLAN queries | `query_lua_artifacts_json` | `fetch_query_lua_artifacts` | `wlan_associations`, `wlan_station_state_snapshots`, SIEM events, raw search | `wlan_stations`, `wlan_known_devices`, `wlan_radios`, guest state. | Unofficial and firmware-dependent; typed rows are snapshot/state evidence. |
| `query.lua` network/security queries | `query_lua_artifacts_json` | `fetch_query_lua_artifacts` | `wan_port_mappings`, `security_advisories`, SIEM events/correlations, raw search | Port sharing, DNS/DHCP/routes, user rights, VPN/WireGuard lists. | Unofficial and firmware-dependent. |
...
| `data.lua` WLAN/network/security pages | `data_lua_pages_json` | `fetch_data_lua_pages` | `wlan_associations`, `wlan_station_state_snapshots`, `dhcp_leases`, `network_status_snapshots`, SIEM events, raw search | Pages include `wlanSta`, `wlanMonitor`, `wlanRadar`, `wlanMesh`, `netDev`, `vpn`, `wireguard`, `remoteAccess`, `diagnosisSecurity`. | Page schemas vary; unsupported fields remain searchable as raw/generic evidence. |
...
| `support_data_txt` section `STATION_LIST` | `support_data_txt` | `parse_support_wifi_observations` | `wlan_station_state_snapshots`, `wlan_station_intervals`, `wifi_connections`, SIEM events/correlations, investigation rows | Known WLAN station state, connect/disconnect history, counters, reason codes, and duration. | Not guaranteed complete; retained diagnostic history varies by firmware. |
| `support_data_txt` section `WLAN_EVENTS` | `support_data_txt` | `parse_support_wifi_observations` | `wlan_event_details`, `wifi_connections`, SIEM events | AP-side WLAN event table with event ID, band, rate, channel, and detail values. | Event IDs still need model/firmware interpretation. |
...
| Telephony `X_AVM-DE_OnTel` paths | `call_list_xml`, `phonebooks_xml_json` | `fetch_telephony_exports` | `telephony_records`, SIEM events, raw search, support findings | Call and phonebook context. | High personal-data sensitivity. |
| AHA HTTP commands | `aha_device_list_xml`, `aha_switch_list_txt`, `aha_device_stats_json` | `fetch_aha_artifacts` | `aha_device_states`, `network_status_snapshots`, SIEM events, raw search, support findings | Smart-home device inventory/state. | Only relevant if FRITZ!DECT/AHA devices exist. |
